...
BB8Adaptor.prototype.connect = function connect(callback) {
    ...
    ble.on("discover", function (peripheral) {
        Logger.debug("BB8Adaptor.connect.discover", peripheral.uuid);
        if (peripheral.uuid === this.uuid) {
            ble.stopScanning();
            self.peripheral = peripheral;

            peripheral.connect(function (connectError) {
                Logger.debug("BB8Adaptor.connect.discover.connect", peripheral.uuid);
                if (connectError) {
                            ...
                            function (seriesCallback) {
                                async.each(
                                    ...
                                    function (service, serviceCallback) {
                                        //Logger.debug("BB8Adaptor.connect.discover.service", peripheral.uuid, service.uuid);
                                        ...
                                        service.discoverCharacteristics([], function (charError, chars) {
                                            if (charError) {
                                                ...
                                                for (var i = 0, j = chars.length; i < j; i++) {
                                                    var char = chars[i];
                                                    //Logger.debug("BB8Adaptor.connect.discover.service.characteristic", peripheral.uuid, service.uuid, char.uuid);
                                                ...
                                                async.each(
                                                    notifiers,
                                                    function(notifier, notifierCallback) {
                                                        if (notifier.uuid == ROBOT_NOTIFY) {
                                                            Logger.debug("notifier", notifier.uuid);
                                                            notifier.on('read', function (data, isNotification) {
                                                                Logger.debug("read", notifier.uuid, data, isNotification);
                                                            });

                                                            notifier.on("data", function(data, isNotification) {
                                                                Logger.debug("data",notifier.uuid, data, isNotification);
                                                            });

                                                            notifier.on("notify", function(state) {
                                                                Logger.debug("notify",notifier.uuid, state);
                                                            });

                                                            notifier.notify(true, function (notifyError) {
                                                                Logger.debug("notier.callback",notifier.uuid);
                                                                if (notifyError) {
                                                                    Logger.error("BB8Adaptor.connect.notify.error", notifyError);
                                                                }
                                                                notifierCallback(notifyError);
                                                            });
                                                        }
                                                        else {
                                                            notifierCallback();
                                                    ...
                                                    function(eachNotifierError) {
                                                        serviceCallback(eachNotifierError);
                                                    }
                                                );
